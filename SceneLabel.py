...
class SceneLabel(Scene):
    def __init__(self, master=None, **kwargs):
        ...
        self.__blob_detector.bind_blob_manager(self.master.blob_manager)
        self.__blob_detector.bind_filter_processor(self.master.filter_processor)
    ...
    def create_styles(self):
        ...
        style = ttk.Style()
        style.configure("DarkMode.TFrame", background="gray23")
        ...
        style.configure("Bold.TLabel", font=("Helvetica", 12, "bold"), background="gray23", foreground="white")
    ...
    def __create_widgets(self):
        ...
        self.__right_widget_manager.add_widget("detect_blobs", ttk.Button, text="Detect Blobs", command=self.__detect_blobs, style="Pg.TButton")
        
        #Filter options
        self.__right_widget_manager.add_widget("filter_parameters", ttk.Label, text="Filter Parameters", style = "Bold.TLabel")
        self.filter_var = tk.BooleanVar(value=False)
        self.__right_widget_manager.add_widget("show_filtered", ttk.Checkbutton, text="Show Filtered Image", command=self.__toggle_renderer, variable=self.filter_var, style="Pg.TCheckbutton")
        ...
        self.__bottom_widget_manager.add_widget("coord", ttk.Label, text="", layout_options={'side': 'left', 'padx': 10}, style="Pg.TLabel")
        self.__bottom_widget_manager.add_widget("scale", ttk.Label, text="", layout_options={'side': 'left', 'padx': 10}, style="Pg.TLabel")
        #
        ...
        self.show_blob_var = tk.BooleanVar(value=True)
        self.__bottom_widget_manager.add_widget("show_blobs", ttk.Checkbutton, text="Show Blobs", layout_options={'side': 'left', 'padx': 10}, variable=self.show_blob_var, command=self.__show_blobs, style="Pg.TCheckbutton")

        self.__bottom_widget_manager.add_widget("tools", ttk.Combobox, values=["Cursor", "Circle"], layout_options={'side': 'left', 'padx': 10}, style="Pg.TCombobox")
        #
        self.__bottom_widget_manager.get_widget("tools").current(0)  # set default
        self.__bottom_widget_manager.get_widget("tools").bind("<<ComboboxSelected>>", self.change_tool)                                            
        self.__bottom_widget_manager.add_widget("fit_to_screen", ttk.Button, text="[ Fit to Screen ]", layout_options={'side': 'left', 'padx': 10}, command=self.fit_to_screen, style="Pg.TButton")
    ...
    def change_tool(self, event):
        ...
        self.__current_tool = selected

        
    def __select_blobs_in_rect(self):
        if not self.__ctrl_pressed:
            self.master.blob_manager.clear_selection()
        ...
        for blob in self.master.blob_manager.get_blobs():
            ...
            if((blob_center[0] + radius <= x1) and (blob_center[0] - radius >= x0) and (blob_center[1] + radius <= y1) and (blob_center[1] - radius >= y0)):
                self.master.blob_manager.select_blob(blob)
    ...
    def __set_drawing(self, cx, cy):
        if not self.master.layer_renderer.is_fg_loaded():
            return

        if self.__current_tool.lower() == "cursor":
        ...
        elif self.__current_tool.lower() == "circle":
            ...
            if self.__start_drawing:
                ...
                if self.__radius > 0:
                    self.master.blob_manager.add_blob(self.__centre[0], self.__centre[1], self.__radius)
                    self.__radius = 0
                    self.canvas.redraw()
        else:
    ...
    def __draw_new_blob(self):
        ...
        thickness = self.master.blob_manager.get_thickness()
    ...
    def __delete_selected_blob(self,event):
        if self.__current_tool.lower() == "circle":
            return
        
        self.master.blob_manager.delete_selected_blobs()
        self.canvas.redraw()
    ...
    def __sDrag(self, cx, cy):
        ...
        if self.__is_dragging:
            ...
            for selected_blob in self.master.blob_manager.get_selected_blobs():
    ...
    def __sSelection(self, cx, cy):
        ...
        half_thickness = self.master.blob_manager.get_thickness()/2.0
        ...
        for blob in self.master.blob_manager.get_blobs():
            ...
            if( (abs(mouse_pos[0] - blob_center[0]) <= radius + half_thickness) and (abs(mouse_pos[1] - blob_center[1]) <= radius + half_thickness) and ((abs(mouse_pos[0] - blob_center[0]) >= (radius/1.4142) - half_thickness) or (abs(mouse_pos[1] - blob_center[1]) >= (radius/1.4142)-half_thickness))):
                ...
                if self.__ctrl_pressed:
                    self.master.blob_manager.toggle_selection(blob)
                else:
                    if not self.master.blob_manager.is_selected(blob):
                        self.master.blob_manager.clear_selection()
                        self.master.blob_manager.select_blob(blob)
    ...
    def __sRender(self, orig_top_left, orig_bottom_right, scale):
        ...
        if self.__use_filter_renderer:
            self.master.filter_processor.render(orig_top_left, orig_bottom_right, scale)
            current_render, shift = self.master.filter_processor.get_render()
        else:
            self.master.layer_renderer.render(orig_top_left, orig_bottom_right, scale, self.fg_opacity)   
            current_render, shift = self.master.layer_renderer.get_render()
    ...
    def __render_blobs(self, scale):
        ...
        text = "Total Blobs: " + str(len(self.master.blob_manager.get_blobs()))
        self.__right_widget_manager.get_widget("total_blobs").config(text=text)

        text = "Selected Blobs: " + str(len(self.master.blob_manager.get_selected_blobs()))
        ...
        if self.master.blob_manager is not None:
            #image_with_blobs = self.original_image.copy()
            selected_blobs = self.master.blob_manager.get_selected_blobs()
            thickness = self.master.blob_manager.get_thickness()
            #print("total blobs found = " ,len(self.blob_manager.get_blobs()))
            for blob in self.master.blob_manager.get_blobs():
                centre = [blob.x, blob.y]  # Convert to integer for pixel coordinates
                ...
                #radius = radius*scale
                #thickness /= scale
                top_left = [centre[0]-radius, centre[1] - radius]
                bottom_right = [centre[0] + radius, centre[1] + radius]
                x0, y0 = self.canvas.orig_image_to_screen(np.array(top_left))
                x1, y1 = self.canvas.orig_image_to_screen(np.array(bottom_right))
                #x0, y0 = center[0] - radius - thickness, center[1] - radius-thickness
                #x1, y1 = center[0] + radius + thickness, center[1] + radius + thickness
    ...
    def __set_filter_params(self):
        ...
        self.master.filter_processor.set_d(d)
        self.master.filter_processor.set_sigma_color(sigma_color)
        self.master.filter_processor.set_sigma_space(sigma_space)
    ...
    def __on_opacity_change(self, val):
        self.fg_opacity = float(val)
        #self.master.layer_renderer.set_opacity(float(val))
        self.canvas.redraw()
    ...
    def __toggle_renderer(self):
        ...
        if self.__use_filter_renderer:
            ...
            self.__bottom_widget_manager.disable_widget("tools")
            ...
            self.__bottom_widget_manager.enable_widget("tools")
